...
class CentraleSol(Entite):
    ...
    def idle(self):
        if self.nbIngenieur >= 1 :
            self.brain.setState(self.production)
    def production(self):
        if cycles%2 == 0 : self.spawn(self.produit)
        if self.nbIngenieur < 1 :
            self.brain.setState(self.idle)
...
class Champ(Entite):
    ...
    def idle(self):
        ...
        if cycles%6 == 0 : self.spawn(Pollinisateur, self.id)
    ...
    def culture(self):
        ...
        if cycles%12 == 0 : self.spawn(Pollinisateur, self.id)
...
class Foret(Entite):
    ...
    def idle(self):
        ...
        if cycles%3 == 0: self.spawn(Arbre, self.id)
        if self.nbTravailleur >= 2:
            self.brain.setState(self.production)
